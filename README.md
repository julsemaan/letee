...
- a styled `+` button before the existing right-side status content; clicking it creates a window in the active pane's current directory on tmux 3.4+

The overlay owns the tmux `MouseDown1Status` binding. Normal inner window-tab clicks still select the clicked window, while the `+` button creates and selects a new window in the active pane's current directory. This replaces any custom binding you have for that mouse event. letee prefixes `status-right` with the button and retains its existing content. No other keybindings, prefix, indexes, history settings, or `status-left` content are changed. The overlay ships as a plain tmux configuration file with native settings only: no TPM plugins, fonts, scripts, or network access.
...
- left-click inner window tabs: select the clicked window; click the `+` button before the right-side status content to create and select a new window in the active pane's current directory (tmux 3.4+)
